...
void initialize_control_loop(void) {
    ...
    debug_text(" RUNFQ=");
    debug_float("runfq", &actual_frequency, 5);
    debug_text(" ERRMX=");
    debug_boolean("control_errmx", &err_mutex_mishandle);
    debug_text(" MAXSV=");
    debug_servo_velocity("maxsv", &maximum_low_speed);
    debug_text(" MAXHV=");
    debug_servo_velocity("maxhv", &maximum_high_speed);
    debug_text(" ");
    void *token = debug_with_mutex(position_mutex);
    debug_text(" DPOS=");
    debug_servo_point("dpos", &driven_position);
    debug_text(" IPOS=");
    debug_servo_point("ipos", &inferred_position);
    debug_text(" TPOS=");
    debug_servo_point("tpos", &target_position);
    debug_text(" ATP=");
    debug_boolean("atp", &at_target_position);
    debug_text(" SM=");
    debug_boolean("sm", &slow_mode);
    debug_text(" ATPN=");
